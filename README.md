...
## Description
A PowerShell script that recursively scans a root path for files with non-ASCII characters (emojis). It renames them by stripping the emojis and cleaning up resulting whitespace or leading/trailing dots.
...
## Configuration & Usage
Set `$rootPath` to the drive or folder you want to scan. Use `$dryRun = $true` first to see what will happen before setting it to `$false` to apply changes.

1. Open the file in a text editor.
2. Locate the configuration section at the top.
3. Modify the paths and variables to match your environment.
4. Run the script: `python UnEmoji.ps1` (or `powershell ./UnEmoji.ps1`)
